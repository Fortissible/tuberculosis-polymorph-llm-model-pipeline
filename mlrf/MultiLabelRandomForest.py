...
from mlrf.DecisionTree import Tree
...
class RandomForestClassifier(object):
    def __init__(self, n_estimators=10, max_depth=-1, min_samples_split=2, min_samples_leaf=1,
        ...
        self._colsample_bytree_count = None  # resolved per-fit column sample size
    ...
    def fit(self, datasets, targets):
        ...
        self._colsample_bytree_count = self._resolve_colsample_bytree(len(datasets[0].columns))
    ...
    def _resolve_colsample_bytree(self, n_features):
        """Resolve column sampling mode to an explicit feature count."""
        mode = self.colsample_bytree
        if mode == "sqrt":
            count = int(n_features ** 0.5)
        elif mode == "log2":
            count = int(math.log(n_features)) if n_features > 1 else 1
    ...
    def _parallel_build_trees(self, datasets, targets, random_state):
        ...
        """
        subcol_index = random.sample(datasets[0].columns.tolist(), self._colsample_bytree_count)
