...
def _ensure_metric_dir(metric_type):
    """Create and return the directory for a given metric category."""
    metric_dir = METRICS_ROOT / metric_type
    metric_dir.mkdir(parents=True, exist_ok=True)
    return metric_dir

def get_metric_path(metric_type, filename):
    """Return the full path for a metric artifact, ensuring directories exist."""
    target_dir = _ensure_metric_dir(metric_type)
    return target_dir / filename

def _save_metric_figure(fig, filename, metric_type):
    """Persist a matplotlib figure inside the metrics directory hierarchy."""
    output_path = get_metric_path(metric_type, filename)
...
def metric_confusion_matrix(y_actual_class, y_pred_class, figure_name, visualize=False):
    ...
    if visualize:
        ...
        plt.title(f'Confusion matrix {figure_name}', fontsize=11)
        filename = f'Confusion matrix {figure_name}.png'
        _save_metric_figure(fig, filename, "confusion_matrix")
        plt.show()
        plt.close(fig)
...
def metric_roc_auc(y_test, y_pred_prob, model_name="", model_type="MLRF"):
    ...
    filename = f'{model_type}-{model_name} ROC Curve.png'
    _save_metric_figure(figure, filename, "roc_curve")
    plt.show()
    plt.close(figure)
